...
Player.prototype.allin = function() {
	console.log('Player ' + this.name + ' ALL-IN : ' + this.chips);
...
Player.prototype.callOrCheck = function() {
	...
	if (diff > 0) {
		...
		console.log('Player ' + this.name + ' CALL : ' + diff);
	} else {
		this.lastAction = "check";
		console.log('Player ' + this.name + ' CHECK');
	}
	this.game.incrementPlayerTurn();
	this.game.checkForNextRound();
};
...
Player.prototype.raise = function(amount) {
	...
	console.log('Player ' + this.name + ' Raises : ' + (diff + amount));
